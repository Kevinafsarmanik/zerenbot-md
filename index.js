        ...
        try {
            ...
            for (let num of participants) {
                ...
                try {
                    ...
                    ppuser2 = await hisoka.profilePictureUrl(num, 'image')
                } catch {
                    ppuser = 'https://i.ibb.co/2ZhLgWQ/tetot.jpg'
                    ppuser2 = 'https://i.ibb.co/2MJfpv0/Sayonara-no-Asa.jpg'
                }
                ...
                if (anu.action == 'add') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Hallo @${num.split("@")[0]}\nSelamat Datang di ${metadata.subject}\n\nSemoga Betah Ya Di Group Ini` })
                } else if (anu.action == 'remove') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser2 }, contextInfo: { mentionedJid: [num] }, caption: `Menyerah pada rindu karena tidak mungkin mengembalikan waktu.\n*Sayonara 👋🏻* @${num.split("@")[0]}\nLeaving To ${metadata.subject}` })
                }
            }
        } catch (err) {
            console.log(err)
